...
if [ $3 = "exp" ]; then
  echo "combine -M MaxLikelihoodFit ${workspace} -m ${mass} --expectSignal=1 -t -1 --rMin=-2 --rMax=4 --robustFit=1 --X-rtd FITTER_DYN_STEP -n hWW${tag} | tee mlf${tag}.txt"
        combine -M MaxLikelihoodFit ${workspace} -m ${mass} --expectSignal=1 -t -1 --rMin=-2 --rMax=4 --robustFit=1 --X-rtd FITTER_DYN_STEP -n hWW${tag} | tee mlf${tag}.txt;
  mv higgsCombinehWW${tag}.MaxLikelihoodFitExp.mH${mass}.root mlfExp${tag}.txt $baseDir/$mass/;
