...
if [ $4 = "exp" ]; then
  ...
  PARAML="--noFitAsimov";
  ADDTEXT="_exp";
  PARAMGOF="--setParameters r_s0=1 -t 500";
elif [ $4 = "expnosig" ]; then
  PARAM="--setParameters r_s0=0 -t -1"; #--toysFrequentist
  PARAML="--noFitAsimov";
  ADDTEXT="_expnosig";
  PARAMGOF="--setParameters r_s0=0 -t 500";
elif [ $4 = "expnosig_postfit" ]; then
  PARAM="--setParameters r_s0=0 -t -1 --toysFrequentist";
  PARAML="--noFitAsimov";
  ADDTEXT="_expnosig_postfit";
  PARAMGOF="--setParameters r_s0=0 -t 500";
fi
...
if [ $3 = "mu" ]; then
  ...
  combineTool.py -M Impacts -d workspace_$2.root -m 125 -n $2${ADDTEXT} --robustFit 1 --X-rtd FITTER_DYN_STEP --rMin=-2 --rMax=2 --doInitialFit $PARAM;
  combineTool.py -M Impacts -d workspace_$2.root -m 125 -n $2${ADDTEXT} --robustFit 1 --X-rtd FITTER_DYN_STEP --rMin=-2 --rMax=2 --doFits --parallel 5 $PARAM;
 ...
 #combine -M FitDiagnostics workspace_$2.root -n $2${ADDTEXT} --plots --saveNorm --saveShapes --saveWithUncertainties --saveOverall --ignoreCovWarning --rMin=-2 --rMax=3 --robustFit=1 --X-rtd FITTER_DYN_STEP $PARAM $5
  combine -M FitDiagnostics workspace_$2.root -n $2${ADDTEXT}         --saveNorm --saveShapes --saveWithUncertainties --saveOverall --keepFailures --ignoreCovWarning --rMin=-2 --rMax=3 --robustFit=1 --X-rtd FITTER_DYN_STEP $PARAM $5
