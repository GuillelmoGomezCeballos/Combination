...
//
// Arguments : 
//  * TString mlfit : name of the root file returned by combine
...
void printaline(int index, TString nuiname, Double_t centralval, Double_t uncert){
...
void plotPostNuisance_combine(TString mlfit="mlfit.root", double MaxUncertainty = 0.9, 
    ...
    for(int i=0; i<nbins-2; i++) {
        if(central[i] <=100000){
          h1->SetBinContent(count+2, central[i]);
          h1->SetBinError(count+2, uncert[i]); 
          h1_ref->SetBinContent(count+2, central[i]);
          h1_ref->SetBinError(count+2, 1.);
	  count++;
	}
