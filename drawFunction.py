...
def drawLimits(limits,tag):
    ...
    #texCMS.Draw()
    if tag.find('shape')>-1: modeLabel = 'H#rightarrow WW #rigtharrow 2l2#nu, 0/1/2-jets (BDT)'
    elif tag.find('cut')>-1: modeLabel = 'H#rightarrow WW #rigtharrow 2l2#nu, 0/1/2-jets (cut)'
    else                   : modeLabel = 'H#rightarrow WW #rigtharrow 2l2#nu, 0/1/2-jets'
    texMode = TLatex(120, 0.8*frame.GetMaximum(), modeLabel)    
    texMode.SetTextSize(14)
    #texMode.Draw()
    if tag.find('7')>-1: lumi = 'L=4.9 fb^{-1}'
    elif tag.find('8')>-1: lumi = 'L=12.1 fb^{-1}'
    elif tag.find('3l')>-1: lumi='L=4.9 fb^{-1} (7 TeV) + 5.1 fb^{-1} (8 TeV)'
    else: lumi='L=4.9 fb^{-1} (7 TeV) + 3.6 fb^{-1} (8 TeV)'
...
def drawBestFit(limits,tag):
    ...
    if tag == '7TeV':
        text.append(TLatex(xCorner,yCorner-0.25,'#sqrt{s} = 7 TeV, L = 4.9 fb^{-1}'))
    elif tag == '8TeV':
        text.append(TLatex(xCorner,yCorner-0.25,'#sqrt{s} = 8 TeV, L = 5.1 fb^{-1}'))
    else:
        text.append(TLatex(xCorner,yCorner-0.25,'#sqrt{s} = 7 TeV, L = 4.9 fb^{-1}'))
        text.append(TLatex(xCorner,yCorner-0.5,'#sqrt{s} = 8 TeV, L = 5.1 fb^{-1}'))
